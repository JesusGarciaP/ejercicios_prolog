...
signos(Dia,Mes,Signo) :- horoscopo(Signo,D1,M1,D2,M2), ((Mes=M1,Dia>=D1,mes(M1,D,_),Dia=<D);(Mes=M2,Dia=<D2, Dia>0)).
rangos(D1,M1,D2,M2) :- (M1=<M2+1), (O is (M1+1)), signo(D1,M1,S), write(S), nl, rangos(D1,O,D2,M2).