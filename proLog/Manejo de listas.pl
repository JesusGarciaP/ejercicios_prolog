/* miembro(Elem,Lista) <- el término Elem pertenecea la lista Lista */ 
miembro(X,[X|_]). 
...
/* ultimo(Elem,Lista) <- Elem es el último elementode la lista Lista */ 
ultimo(X,[X]). 
...
/* union(L1,L2,L3) <- L3 es la lista-conjunto uniónde L1 y L2 */ 
union([],L,L). 
